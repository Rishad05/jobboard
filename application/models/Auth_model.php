...
class Auth_model extends CI_Model
{
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $data['parse_data'] = array(
                        ...
                        'email' => $user->email,
                        'reset_password_link' => base_url('auth/reset_password_member/' . $user->forgotten_password_code),
                        'site_link' => base_url(),
                        'site_name' => $this->Settings->site_name,
                        'logo' => base_url('themes/frontend/bos_theme/assets/images/mail_logo.png'),
                    );


                    $message = $this->load->view($this->frontend_theme . 'email_templates/passenger_forgot_password2', $data, true);
    ...
    public function login($identity, $password, $remember = false)
    {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, first_name, last_name, created_on, last_login, last_ip_address, avatar, gender, group_id, applicant_id')
        ...
        //echo 'query result' . $query->num_rows();
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === true) {
                ...
                if ($user->active != 1) {
                    ...
                    $support_url = "<a href=" . base_url('contact-us') . ">click here </a>";

                    //$this->set_error('login_unsuccessful_not_active');
                    $this->set_error('Login unsuccessfull! Your account is not activate yet, please ' . $support_url . ' to contact our support');
    ...
    public function set_session($user)
    {
        ...
        if ($user->group_id != 1 && $user->group_id != 2) {
            $session_data['member_id'] = $user->id;
        } else {
    ...
    public function membearInfo($id)
    {
        $this->db->select('users.*, users.id as user_id, user_details.*, user_details.id as detailsId, user_details.avatar as p_photo');
    ...
    public function quInfo($id)
    {
        $this->db->where('user_id', $id);
        $n = $this->db->get('user_qualification');
        if ($n->num_rows()) {
            return $n->result();
        } else {
            return array();
