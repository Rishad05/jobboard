...
class Auth extends MY_Controller {
    ...
    function users() {
        ...
        $bc = array(array('link' => '#', 'page' => lang('User_List')));
        $meta = array('page_title' => lang('Administrator_List'), 'bc' => $bc);
    ...
    function members() {
        ...
        $this->data['users'] = $this->site->getAllMembers();

        $bc = array(array('link' => '#', 'page' => lang('Member_List')));
        $meta = array('page_title' => lang('Member_List'), 'bc' => $bc);
        $this->data['page_title'] = lang('Member_List');
        $this->page_construct('auth/members', $this->data, $meta);
    }
    ...
    function login($m = NULL) {
        ...
        if ($this->form_validation->run() == true) { 
            ...
            $group = $this->site->wheres_row('users', array('email'=>$this->input->post('identity')));
            if($group){ 
    ...
    function member_login()
    {
        ...
        $bc = array(array('link' => '#', 'page' => 'Member Login'));
        $meta = array('page_title' => 'Member Login', 'bc' => $bc);
    ...
    function member_registration()
    {
        ...
        if ($this->form_validation->run() == true && $uid = $this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify)) {  
           ...
           $message = $this->load->view($this->frontend_theme.'email_templates/registration', $this->data,TRUE); 
            ...
            $bc = array(array('link' => '#', 'page' => 'Member Login'));
            $meta = array('page_title' => 'Member Registration', 'bc' => $bc);
    ...
    function create_member()
    {
        ...
        if ($this->form_validation->run() == true) { 
            ...
            if($_FILES['userfile']['size'] > 0) { 
              ...
              if (!$this->upload->do_upload()) { 
                $error = $this->upload->display_errors(); 
                $this->session->set_flashdata('error', $error);
                $this->form_validation->set_rules('image_size', lang("name"), 'required'); 
                //die("<script type='text/javascript'>setTimeout(function(){ window.top.location.href = '" . (isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : site_url('welcome')) . "'; }, 10);</script>"); 
                  }
             $photo = $this->upload->file_name ;
            }else{$photo='';}
        ...
        if ($this->form_validation->run() == true && $uid = $this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify)) {  
           ...
           $message = $this->load->view($this->frontend_theme.'email_templates/registration', $this->data,TRUE); 
            ...
            $this->session->set_flashdata('message','Membership registration successfull!'); 

            $admin_subject = 'A new member has been registered at www.bosbd.org';
            $admin_message = 'Member details'."\n";
            $admin_message .= 'Name: '.$this->input->post('name')."\n";
            $admin_message .= 'Email: '.$this->input->post('email')."\n";
            $admin_message .= 'BMDC Registration No: '.$this->input->post('bmdc_egistration_no')."\n\n";

            $this->tec->send_email($this->Settings->default_email, $admin_subject, $admin_message, $email, $this->input->post('name'), NULL,NULL, NULL); 
            ...
            $this->db->select('*');
            $this->db->where_in('id', array(3,4,5,6));
            $this->db->from('groups');
            $q = $this->db->get(); 
            $this->data['groups'] = $q->result(); 


            $this->data['page_title'] = lang('Add_New_Member'); 
            $bc = array(array('link' => site_url('users'), 'page' => lang('users')), array('link' => '#', 'page' => lang('Add_Member')));

            $meta = array('page_title' => lang('Add_New_Member'), 'bc' => $bc); 

            $this->page_construct('users/create_member', $this->data, $meta);  
        }
    }
    function update_member($id=NULL, $details_id=NULL)
    {
       ...
       // $this ->form_validation -> set_rules('password', 'Password', 'required|max_length[15]|min_length[8]');
        ...
        if ($this->form_validation->run() == true) { 
            ...
            if($this->site->updateData('user_details', $details, array('id'=>$details_id))){ 

                $this->site->updateData('users', array('group_id'=>$this->input->post('group')), array('id'=>$id));

                $this->site->delete('user_qualification', array('user_id'=>$id));
           ...
           $user_active =  $this->auth_model->membearInfo($id)->active;
           if($user_active == 1 && $this->input->post('status') == 0){ 
                $this->site->updateData('users', array('active'=>$this->input->post('status')), array('id'=>$id));
           }
            if($user_active == 0 && $this->input->post('status') == 1){ 
                 
                $this->site->updateData('users', array('active'=>$this->input->post('status')), array('id'=>$id));

                $subject = 'Your account has been activated at www.bosbd.org';
                $message = 'Congratulations, your account activated'."\n";
                $message .= 'Please login <a href="http://timesavedhaka.com/bos/member-login">here </a>'."</br>"; 

                $this->tec->send_email($this->input->post('email'), $subject, $message, $this->Settings->default_email, 'www.bosbd.org', NULL,NULL, NULL); 
            }
            ...
            $this->data['info'] = $this->auth_model->membearInfo($id);
            $this->data['qinfo'] = $this->auth_model->quInfo($id); 
            
            $this->data['page_title'] = lang('Update_Member_Information'); 
            $bc = array(array('link' => site_url('users'), 'page' => lang('users')), array('link' => '#', 'page' => lang('Edit')));

            $meta = array('page_title' => lang('Update_Member_Information'), 'bc' => $bc); 

            $this->page_construct('users/update_member', $this->data, $meta);  
        }
    }

    function delete_member($user_id=NULL, $member_id=NULL)
    {
        if(DEMO) { 
        ...
        if ($this->site->delete('user_details', array('id' => $member_id))) { 
            $this->site->delete('users', array('id' => $user_id ));
            $this->session->set_flashdata('message', 'Member delete successfully'); 
    ...
    {
       $identity = $this->site->wheres_row('users', array('email'=>base64_decode($key)));
        if($identity){
            if($this->site->updateData('users', array('varify' => 1), array('email'=>base64_decode($key)))){
...
function create_user() {
    ...
    if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify)) {
        ...
        $meta = array('page_title' => lang('Add_New_Administrator'), 'bc' => $bc);
