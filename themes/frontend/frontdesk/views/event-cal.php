                ...
                if(($cb >= $calendar['currentMonthFirstDay']+1 || $calendar['currentMonthFirstDay'] == 7) && $cb <= ($calendar['totalDaysOfMonthDisplay'])){ 
                    ...
                    if(!empty($events)){ 
                        $eventData = array_filter($events, function ($var) use ($currentDate) { 
                            return ($var['start_date'] == $currentDate); 
                        }); 
                        ...
                        $eventTitle = !empty($eventData['title'])?$eventData['title']:NULL;
                        $eventlink = !empty($eventData['slug'])?$eventData['slug']:NULL; 
                        $eventID = !empty($eventData['id'])?$eventData['id']:NULL; 
                    } 
                    $event = $this->event->getEvent($currentDate);                     
                    ...
                    // Date cell 
                    echo '<span class="dfg">'.$dayCount; 
                    if($event){
                        echo '<ul>';
                        foreach ($event  as $key => $even ) {
                            if($even['slug']){
                                echo '<li><a target="_blank" href="'.base_url("training/details/"). $even['slug'].'"> ' .$even['title'] .'</a></li>';
                            }else{
                                echo '<li><a target="_blank" href="'.base_url("training/detail/").$even['id'].'"> ' .$even['title'] .'</a></li>';
                            } 
                        }
                        echo '</ul>';
                    }
                    ...
                    /*if($eventlink){
                        echo $dayCount. ' <a target="_blank" href="'.base_url("training/details/"). $eventlink.'"> ' .$eventTitle .'</a>'; 
                    }else{
                        echo $dayCount . '<a target="_blank" href="'.base_url("training/detail/"). $eventID.'"> ' .$eventTitle .'</a>'; 
                    }*/
